...
from memory import Buffer, Memory, HMemory
from util import *
...
def evaluate(actor, env, buffer=None, n_episodes=1, random=False, noise=None, render=False):
    """
    ...
    for _ in range(n_episodes):
        ...
        while not done:
            ...
            if buffer is not None:
                buffer.store((obs, n_obs, action, reward, done_bool))
            obs = n_obs
...
if __name__ == "__main__":
    ...
    # ES parameters
    parser.add_argument('--es_pop_size', default=6, type=int)
    parser.add_argument('--rl_pop_size', default=4, type=int)
    ...
    parser.add_argument('--hmem_size', default=50000, type=int)
    parser.add_argument('--n_noisy', default=0, type=int)
    parser.add_argument('--sp', default=0.2, type=float)
    ...
    hmemory = HMemory(args.hmem_size, state_dim, action_dim)
    ...
    # RL actor population
    rl_params = []
    for _ in range(args.rl_pop_size):
        rl_params.append(Actor(state_dim, action_dim, max_action, args).get_params())
    ...
    es = sepCEM(actor.get_size(), mu_init=actor.get_params(), sigma_init=args.sigma_init, damp=args.damp, damp_limit=args.damp_limit,
                pop_size=args.es_pop_size, antithetic=not args.es_pop_size % 2, parents=1, elitism=args.elitism)
    ...
    total_steps = 0
    actor_steps = 0
    df = pd.DataFrame(columns=["total_steps", "es_average_score", "rl_average_score", "best_train_score", "best_eval_score"])
    ...
    while total_steps < args.max_steps:
        ...
        if total_steps > args.start_steps:
            ...
            for i in range(args.rl_pop_size):

                # set params
                actor.set_params(rl_params[i])
                actor_t.set_params(rl_params[i])
                actor.optimizer = torch.optim.Adam(actor.parameters(), lr=args.actor_lr)

                # critic update
                for _ in tqdm(range(actor_steps // args.rl_pop_size)):
                    if random.random() < args.sp:
                        critic.update(hmemory, args.batch_size, actor, critic_t)
                    else:
                ...
                for _ in tqdm(range(actor_steps // args.rl_pop_size)):
                    if random.random() < args.sp:
                        actor.update(hmemory, args.batch_size, critic, actor_t)
                    else:
                        actor.update(memory, args.batch_size, critic, actor_t)

                # get the params back in the population
                rl_params[i] = actor.get_params()

        actor_steps = 0

        # evaluate all rl actors
        for params in rl_params:

            actor.set_params(params)
            f, steps = evaluate(actor, env, buffer=buffer, n_episodes=args.n_episodes, render=args.render)
            if f >= f_max:
                hmemory.add(buffer.cache_buffer)
            memory.add(buffer.cache_buffer)
            buffer.clear()
            actor_steps += steps
            fitness_rl.append(f)

            # print scores
            prLightPurple('RL Actor fitness:{}'.format(f))

        # evaluate all es actors
        es_params = es.ask(args.es_pop_size)
        ...
        f_max = max(f_max, max(fitness_rl), max(fitness_es))

        # update step counts
        total_steps += actor_steps
        step_cpt += actor_steps

        best_es_actor = fitness_es.index(max(fitness_es))
        best_rl_actor = fitness_rl.index(max(fitness_rl))

        best_actor_params = rl_params[best_rl_actor] if max(fitness_rl) > max(fitness_es) else es_params[best_es_actor]

        if total_steps > args.start_steps:
            all_fitness = fitness_es + fitness_rl
            index_sort = sorted(range(len(all_fitness)), key=all_fitness.__getitem__)
            index_sort.reverse()
            elites, elites_fitness = [], []
            for index in index_sort[:args.es_pop_size]:
                if index < args.es_pop_size:
                    elites.append(es_params[index])
                    elites_fitness.append(fitness_es[index])
                else:
                    elites.append(rl_params[index - 6])
                    elites_fitness.append(fitness_rl[index - 6])
            for i in range(args.es_pop_size):
                es_params[i] = elites[i]
                fitness_es[i] = elites_fitness[i]
            for i in range(args.rl_pop_size):
                rl_params[i] = elites[i]
                fitness_rl[i] = elites_fitness[i]

        # update es
        es.tell(es_params, fitness_es)
        ...
        if step_cpt >= args.period:
            ...
            actor.set_params(best_actor_params)
            f, _ = evaluate(actor, env, buffer=None, n_episodes=args.n_eval, render=args.render)
            prRed('Best Actor Average Fitness:{}'.format(f))

            df.to_pickle(args.output + "/log.pkl")
            res = {"total_steps": total_steps,
                   "es_average_score": np.mean(fitness_es),
                   "rl_average_score": np.mean(fitness_rl),
                   "best_train_score": np.max(fitness_es + fitness_rl),
                   "best_eval_score": f}
            ...
            if args.save_all_models:
                ...
                actor.set_params(best_actor_params)
                actor.save_model(args.output + "/{}_steps".format(total_steps), "actor")
            else:
