...
if __name__ == "__main__":
    ...
    while total_steps < args.max_steps:
        ...
        worst_es_actor = fitness_es.index(min(fitness_es))
        worst_rl_actor = fitness_rl.index(min(fitness_rl))

        best_actor_params = rl_params[best_rl_actor] if max(fitness_rl) > max(fitness_es) else es_params[best_es_actor]

        if total_steps > args.start_steps:
            if fitness_es[worst_rl_actor] < fitness_rl[best_rl_actor]:
                es_params[worst_es_actor] = rl_params[best_rl_actor]; fitness_es[worst_es_actor] = fitness_rl[best_rl_actor]
            if fitness_rl[worst_rl_actor] < fitness_es[best_es_actor]:
                rl_params[worst_rl_actor] = es_params[best_es_actor]; fitness_rl[worst_rl_actor] = fitness_es[best_es_actor]
